...
Public Class binary_search
    ...
    Private Class Question
        Public Property Text As String ' Question text
        Public Property Options As List(Of String) ' List of options
        Public Property CorrectAnswer As String ' Correct answer
    End Class
    ...
    ' List to hold labels representing array indices
    Private indexLabels As New List(Of Label)()
    ' Timer for controlling the search process
    Private searchTimer As Timer
    ' Variables to store low and high indices for binary search
    Private low As Integer = -1
    Private high As Integer = -1
    ' Flag to indicate if the input array is sorted
    ...
    ' Dictionary to store user answers for each question
    ...
    Private Sub BinarySearchForm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ' Scroll the form to the top-left corner
    ...
    Private Sub btnInitialize_Click(sender As Object, e As EventArgs) Handles btnInitialize.Click
        Try
            ...
            ' Convert input elements to integers and check if the list is sorted
            isSorted = True
            ReDim array(elements.Length - 1)
            Dim previousElement As Integer = Integer.MinValue
            For i As Integer = 0 To elements.Length - 1
                If Not Integer.TryParse(elements(i), array(i)) Then
                    UpdateSearchResult("Invalid input. Please enter integers separated by commas.")
                    Return
                End If

                If array(i) < previousElement Then
                    isSorted = False
                End If
            ...
            If Not isSorted Then
                ...
                UpdateSearchResult("The provided list was unsorted and has been sorted.")
            End If
    ...
    Private Sub InitializeArrayVisualization()
        ' Reset UI elements
        txtInput.Clear()
        pnlBinarySearch.Controls.Clear()
        pnlBinarySearch.Controls.Add(lblSearchResult)
        arrayLabels.Clear()
        indexLabels.Clear()
        ...
        Dim maxWidth As Integer = pnlBinarySearch.ClientSize.Width
        ...
        For i As Integer = 0 To array.Length - 1
            ...
            ' Add label to panel
            pnlBinarySearch.Controls.Add(label)
            ...
            ' Add index label to panel
            pnlBinarySearch.Controls.Add(indexLabel)
        ...
        ' Hide search result label if array is sorted
        If isSorted Then
            lblSearchResult.Visible = False
        End If
        ' Enable search and reset buttons
    ...
    Private Sub btnReset_Click(sender As Object, e As EventArgs) Handles btnReset.Click
        Try
            ...
            pnlBinarySearch.Controls.Clear()
            pnlBinarySearch.Controls.Add(lblSearchResult)
            ' Reset search process and UI elements
            'lblSearchResult.Visible = False
            'low = 0
            'high = array.Length - 1
        ...
        Try
            ...
            If low <= high Then
                For i As Integer = low To high
                    DrawBorder(arrayLabels(i), Color.SteelBlue)
                Next
                Dim mid As Integer = low + (high - low) \ 2
                arrayLabels(mid).BackColor = Color.SteelBlue

                If array(mid) = searchElement Then
                    arrayLabels(mid).BackColor = Color.Yellow
                    searchTimer.Stop() ' Stop the timer after processing all elements
                    UpdateSearchResult("Element " & searchElement & " found at index " & mid & ".")
                    low = 0
                    high = array.Length - 1
                    btnStop.Text = "Stop"
                    txtInput.ReadOnly = False ' Enable the text input field
                    ...
                    'txtInput.Enabled = True ' Enable the text input field
                    Exit Sub
                ElseIf array(mid) < searchElement Then
                    low = mid + 1
                Else
                    high = mid - 1
                End If
    ...
    Private Sub DrawBorder(label As Label, color As Color)
        Dim borderThickness As Integer = 5 ' Adjust as needed
        Using g As Graphics = label.CreateGraphics()
            Using p As New Pen(color, borderThickness)
                g.DrawRectangle(p, New Rectangle(0, 0, label.Width - 3, label.Height - 3))
            End Using
        End Using
    End Sub
    ...
    Private Sub LoadQuizQuestions()
        ...
        Dim query As String = "SELECT serial_no, question, optionA, optionB, optionC, optionD, Correct_Option FROM binary_search"
    ...
    Private Sub DisplayQuestion()
        ...
        ' Calculate the height of the question label
    ...
    Private Sub ShowQuizResults()
        ...
        Dim connectionString As String = "server=localhost;userid=root;password=pwd;database=swe_assign2"
        Dim query As String = "UPDATE users SET binarysearch_progress = GREATEST(IFNULL(binarysearch_progress, 0), @correctAnswers) WHERE username = @user"
    ...
    Private Sub PictureBox3_Click(sender As System.Object, e As System.EventArgs) Handles PictureBox3.Click
        ...
        Profile.PreviousForm = Me ' Pass this form reference to profile page for its 'back' functionality
    ...
    ' Event handler for reset questions button click
